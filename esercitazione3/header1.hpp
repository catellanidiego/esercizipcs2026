...
//utilizzate funzioni template dopo la spiegazione nell'esercitazione 4
template<typename T> requires std::integral<T>
rational<T> operator+(const T& add_int, const rational<T>& add_raz) {
	rational<T> a(add_int, 1);
	a += add_raz;
	return a;
}

template<typename T> requires std::integral<T>
rational<T> operator-(const T& m_int, const rational<T>& s_raz) {
	rational<T> a(m_int, 1);
	a -= s_raz;
	return a;
}

template<typename T> requires std::integral<T>
rational<T> operator*(const T& fatt_int, const rational<T>& fatt_raz) {
	rational<T> a(fatt_int, 1);
	a *= fatt_raz;
	return a;
}

template<typename T> requires std::integral<T>
rational<T> operator/(const T& do_int, const rational<T>& de_raz) {
	rational<T> a(do_int, 1);
	a /= de_raz;
	return a;
}